...
<body style="background-image: url(food.png);">
	...
	<table cellspacing="1" cellpadding="5" style="float: right; margin-top: 15px;">
		<tr>
			<td>
			<input type="button" value="登入" onclick="location.href='http://localhost/foodmap/fm_login.html'">
			</td>
			<td>
			<input type="button" value="註冊" onclick="location.href='http://localhost/foodmap/fm_register.html'">
			</td>
		</tr>
	</table>
	</header>
...
<table>
		<?php
			...
			while($row_result_popu = mysqli_fetch_array($result_popu)){
				...
				?>
				<form name="display" action="fm_vshop_display.php" method="post">
...
<table>
		<?php
			$random_s = "SELECT register.RID, register.PW, shop.name, photo.photo FROM `register`,`shop`,`photo` WHERE shop.SID = photo.PID and register.RID = shop.SID GROUP BY shop.name ORDER BY rand() LIMIT 10";
